...
#KickStart Actions: None
   ...
   #(FOUO)
   #4.B.3.a(5)(b)
   #Synopsis:  Protect the contents of audit trails against unauthorized
...
EOF
   ...
   chmod 600 /var/log/audit/*
      ...
      #(FOUO)
      #4.B.3.a(5)(d)(1)
      #Synopsis: Successful and unsuccessful logons and logoffs. 
      #
      ...
      #Synopsis: Successful and unsuccessful logons and logoffs
      #
      #Kickstart Actions: See Section 4.B.3.a(5)(d)(2)
      #

      #4.B.3.a(5)(d)(2)
      #Synopsis: Accesses to security-relevant objects and directories
      #          including opens, closes, modifications, and deletions.
      #
      ...
      #(FOUO)
      #4.B.3.a(5)(d)(3)
      #Synopsis: Activities at the system console, and other system
      #          accesses by privileged users.
      #
      #KickStart Actions: All authentication attemps will be monitored
      #                   in /var/log/messages.

      if [ -e /etc/syslog.conf.nolock ] ; then
      	   echo "File exists"
      else
      	   cp -p /etc/syslog.conf /etc/syslog.conf.nolock
...
#KickStart Actions: None
   ...
   if [ "`hostname -s`" == "$ORACLE_SERVER" ]; then
   	/sbin/chkconfig portmap on
   fi
   # Turing portmap off on the server running Teamspeak server, breaks Teamspeak
   if [ "`hostname -s`" == "$TEAMSPEAK_SERVER" ]; then
   	/sbin/chkconfig portmap on
   fi
...
if [ -e /etc/audit/auditd.conf.nolock ] ; then
	...
	#Set E-mail Address
	sed s/"action_mail_acct = root"/"action_mail_acct = incfadmin@incf.argoneng.com"/g /etc/audit/auditd.conf > /tmp/tmp1
	cp /tmp/tmp1 /etc/audit/auditd.conf
	rm /tmp/tmp1
fi
...
# Feel free to add below this line. See auditctl man page
      ...
      ##Audits for failed attempts to access files and programs.
      -a exit,always -S mknod -F success=0
      #-a exit,always -S open -F success=0
      -a exit,always -S pipe -F success=0
      -a exit,always -S mkdir -F success=0
      -a exit,always -S creat -F success=0
      -a exit,always -S clone -F success=0
      -a exit,always -S fork -F success=0
      -a exit,always -S vfork -F success=0
      -a exit,always -S umask -F success=0
      #LEAK
      #MOD
      -a exit,always -S truncate -F success=0
      #-a exit,always -S truncate64 -F success=0
      -a exit,always -S ftruncate -F success=0
      #-a exit,always -S ftruncate64 -F success=0

      ##Audit for files and programs deleted by user
      -a exit,always -S unlink -F success=0
      -a exit,always -S rmdir -F success=0
      -a exit,always -w /bin/rm -F success=0
      -a exit,always -S rename -F success=0
      ...
      #  Dev_configure
      #  Dev_create
      -a exit,always -S chown -F success=0
      #-a exit,always -S chown32 -F success!=0
      -a exit,always -S fchown -F success=0
      #-a exit,always -S fchown32 -F success!=0
      -a exit,always -S lchown -F success=0
      #-a exit,always -S lchown32 -F success!=0
      -a exit,always -S chroot -F success=0 -F success!=0
      -a exit,always -S mount -F success=0 -F success!=0
      #-a exit,always -S umount -F success=0 -F success!=0
      -a exit,always -S umount2 -F success=0 -F success!=0
      -a exit,always -w /usr/sbin/pwck -F success=0 -F success!=0
      -a exit,always -S adjtimex -F success=0
      -a exit,always -S settimeofday -F success=0
      -a exit,always -S kill -F success=0
      #Proc_privilege
      -a exit,always -w /bin/chgrp -F success=0
      -a exit,always -w /usr/bin/newgrp -F success=0 -F success!=0
      -a exit,always -w /usr/sbin/groupadd -F success=0 -F success!=0
      -a exit,always -w /usr/sbin/groupmod -F success=0 -F success!=0
      -a exit,always -w /usr/sbin/groupdel -F success=0 -F success!=0
      #Restore imports

      #TCBCK_delete
      -a exit,always -w /usr/sbin/useradd -F success=0 -F success!=0
      -a exit,always -w /usr/sbin/userdel -F success=0 -F success!=0
      -a exit,always -w /usr/sbin/usermod -F success=0 -F success!=0
      -a exit,always -w /usr/bin/chage -F success=0 -F success!=0
      -a exit,always -S reboot -F success=0 -F success!=0
      #User_setenv
      ...
      -w /bin/login -p x
      -a exit,always -w /bin/logout -F success!=0
      -a exit,always -S init_module -F success=0
      -a exit,always -S delete_module -F success!=0
      -a exit,always -w /bin/su -F success=0 -F success!=0

      ##Audit use of privileged commands
      -a exit,always -w /usr/bin/setfacl -F success=0 -F success!=0
      -a exit,always -w /usr/bin/chacl -F success=0 -F success!=0
      -a exit,always -S chmod -F success=0
      -a exit,always -S fchmod -F success=0
      -a exit,always -S link -F success=0
      -a exit,always -S symlink -F success=0
      #  Proc_realgid
      #  Proc_setuserids
...
-w /etc/mail/access -p wa
-w /etc/mail/access.db -p wa
-w /etc/mail/domaintable -p wa
-w /etc/mail/domaintable.db -p wa
-w /etc/mail/helpfile -p wa
-w /etc/mail/local-host-names -p wa
-w /etc/mail/mailertable -p wa
-w /etc/mail/mailertable.db -p wa
-w /etc/mail/Makefile -p wa
-w /etc/mail/sendmail.cf -p wa
-w /etc/mail/sendmail.mc -p wa
-w /etc/mail/submit.cf -p wa
-w /etc/mail/submit.mc -p wa
-w /etc/mail/trusted-users -p wa
-w /etc/mail/virtusertable -p wa
-w /etc/mail/virtusertable.db -p wa
-w /etc/samba/smb.conf -p wa
-w /etc/syslog.conf -p wa
-w /etc/resolv.conf -p wa
-w /etc/nsswitch.conf -p wa
-w /etc/host.conf -p wa
-w /etc/yp.conf -p wa
-w /var/yp/binding -p wa
-w /etc/ldap.conf -p wa
-w /etc/krb5.conf -p wa
-w /etc/initlog.conf -p wa
-w /etc/default/ -p wa
-w /etc/fstab -p wa
-w /etc/auto.master -p wa
-w /etc/auto.misc -p wa
...
#echo "password" >> /etc/grub.conf
chmod 600 /etc/grub.conf

#chmod 0600 /etc/.ICEauthority
